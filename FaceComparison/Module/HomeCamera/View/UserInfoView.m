...
- (void)show
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.frame = CGRectMake(self.frame.origin.x, HFJKSCREEN_HEIGHT - self.frame.size.height + 5, self.frame.size.width, self.frame.size.height);
    }];
...
- (void)makeConstraints
{
    ...
    self.layer.borderColor = [HexColor colorWithHexString:@"2166B6"].CGColor;
    ...
    [self.name mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.nameTap);
    ...
    [self.typeOfWork mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.typeOfWorkTap);
...
- (UILabel *)name
{
    if (!_name) {
        ...
        _name.font = [UIFont fontWithName:@"PingFang SC" size:20];
...
- (UILabel *)typeOfWork
{
    if (!_typeOfWork) {
        ...
        _typeOfWork.font = [UIFont fontWithName:@"PingFang SC" size:20];
...
- (UILabel *)jobNum
{
    if (!_jobNum) {
        ...
        _jobNum.font = [UIFont fontWithName:@"PingFang SC" size:20];
...
- (UILabel *)nameTap
{
    if (!_nameTap) {
        ...
        _nameTap.font = [UIFont fontWithName:@"PingFang SC" size:20];
        _nameTap.text = @"姓名 :";
...
- (UILabel *)typeOfWorkTap
{
    if (!_typeOfWorkTap) {
        ...
        _typeOfWorkTap.font = [UIFont fontWithName:@"PingFang SC" size:20];
...
- (UILabel *)jobNumTap
{
    if (!_jobNumTap) {
        ...
        _jobNumTap.font = [UIFont fontWithName:@"PingFang SC" size:20];
        _jobNumTap.text = @"工号 :";
...
- (UILabel *)score
{
    if (!_score) {
        ...
        _score.textColor = [HexColor colorWithHexString:@"#07D4A1"];
